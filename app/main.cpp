...
// Calculate potential energy between two particle
double calcPotentialEnergy(const Particle &p1, const Particle &p2) {
  ...
  // Check when distance is too small or calculate the energy with itself
...
// Calculate Total energy without it self for a list of particles
...
// Print help messages when user specified
void print_help() {
  ...
  std::cout
      << "  -r, --random          randomGenerator with number of particles"
      << std::endl;
  std::cout << "  -e,                   epsilon" << std::endl;
}

int main(int argc, char *argv[]) {
  // Declare default value when no user specified
  double dt = 0.0001;
  ...
  // Report error when no generator specified
  if (!generator) {
...
#pragma omp parallel for reduction(+:initial_kinetic_energy, initial_potential_energy)
  ...
  // Use OpenMP and avoid error calculation with the copy of the particles
...
#pragma omp parallel for
  ...
  auto total_time1 = std::chrono::duration_cast<std::chrono::milliseconds>(
                         end_time - start_time)
                         .count();
...
#pragma omp parallel for reduction(+:final_kinetic_energy, final_potential_energy)
  ...
  // Print the information to check manually and report in README
  final_total_energy = final_kinetic_energy + final_potential_energy;
  std::cout << "Initial kinetic energy = " << initial_kinetic_energy
            << std::endl;
  std::cout << "Initial potential energy = " << initial_potential_energy
            << std::endl;
  std::cout << "Initial total energy = " << initial_total_energy << std::endl;
  std::cout << "Final kinetic energy = " << final_kinetic_energy << std::endl;
  std::cout << "Final potential energy = " << final_potential_energy
            << std::endl;
  ...
  std::cout << "Timestep = " << dt << ", Total time = " << total_time1
            << " ms, Average time = " << avg_time << " ms" << std::endl;
}