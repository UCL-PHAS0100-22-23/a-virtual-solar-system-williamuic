...
std::vector<Particle> RandomGenerator::generateInitialConditions() {
    std::random_device rd;
    std::mt19937 gen(80);//Developer can change 80, it is a seed control the random generation
    ...
    for (size_t i = 1; i < num_particles_; ++i) {
        ...
        Eigen::Vector3d velocity(-sqrt(1.0 / distance) * cos(angle), sqrt(1.0 / distance) * sin(angle), 0.0);
        Eigen::Vector3d acceleration(0.0, 0.0, 0.0);
