...
std::vector<Particle> create_initial_particles() {
  ...
  // Developer could choose using rd or use a fix number so that every time it
  // will generate same random number
...
TEST_CASE("Particle moves with no acceleration", "[particle]") {
  Particle p(Eigen::Vector3d(1.0, 2.0, 3.0), Eigen::Vector3d(4.0, 5.0, 6.0),
...
TEST_CASE("Particle moves with constant acceleration", "[particle]") {
  Particle p(Eigen::Vector3d(1.0, 2.0, 3.0), Eigen::Vector3d(4.0, 5.0, 6.0),
             Eigen::Vector3d(0.0, 0.0, -9.8), 1.0);
  double dt = 0.1;
  p.update(dt);
  REQUIRE(p.getPosition().isApprox(Eigen::Vector3d(1.4, 2.5, 3.6)));
...
TEST_CASE("Particle orbits around origin with artificial acceleration",
          "[particle]") {
  Particle p(Eigen::Vector3d(1.0, 0.0, 0.0), Eigen::Vector3d(0.0, 1.0, 0.0),
...
TEST_CASE("Particle has correct mass", "[particle]") {
  Particle p(Eigen::Vector3d(1.0, 2.0, 3.0), Eigen::Vector3d(4.0, 5.0, 6.0),
             Eigen::Vector3d(0.0, 0.0, -9.8), 2.0);
...
TEST_CASE("Gravitational force between two particles", "[particle]") {
  ...
  Particle p1(Eigen::Vector3d(0.0, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  Particle p2(Eigen::Vector3d(1.0, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  ...
  Particle p3(Eigen::Vector3d(0.0, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  Particle p4(Eigen::Vector3d(2.0, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
...
TEST_CASE("SumAccelerations for all particles", "[particle]") {
  ...
  Particle p3(Eigen::Vector3d(-1.0, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  particles = {p1, p2, p3};
  p1.SumAccelerations(particles, epsilon);
...
TEST_CASE("Gravitational force edge cases", "[particle]") {
  ...
  Particle p1(Eigen::Vector3d(0.0, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  Particle p2(Eigen::Vector3d(0.0, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  ...
  // Test case 2: Two particles with very small distance and non-zero softening
  // factor
  Particle p3(Eigen::Vector3d(0.0, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  Particle p4(Eigen::Vector3d(1e-10, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  expected_acceleration = Eigen::Vector3d(1e5, 0.0, 0.0);
  epsilon = 1e-5;
  calculated_acceleration = calcAcceleration(p3, p4, epsilon);
  std::cout << "Calculated acceleration: " << calculated_acceleration
            << std::endl; // Print out calculated acceleration
...
TEST_CASE("SumAccelerations edge cases", "[particle]") {
  ...
  p1.SumAccelerations(particles, epsilon);
  Eigen::Vector3d expected_acceleration(0.0, 0.0, 0.0);
  std::cout << "Particle acceleration: " << p1.getAcceleration()
            << std::endl; // Print out particle acceleration

  REQUIRE(p1.getAcceleration().isApprox(expected_acceleration, 1e-5));

  // Test case 2: Three particles in a line with very small distance and
  // non-zero softening factor
  Particle p3(Eigen::Vector3d(0.0, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  Particle p4(Eigen::Vector3d(1e-10, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  Particle p5(Eigen::Vector3d(2e-10, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  particles = {p3, p4, p5};
  epsilon = 1e-5;
  p3.SumAccelerations(particles, epsilon);
  std::cout << "Particle acceleration: " << p3.getAcceleration()
            << std::endl; // Print out particle acceleration
...
// 1.3 Simulator
TEST_CASE("create_initial_particles with correct number of particles",
          "[particle]") {
...
TEST_CASE("create_initial_particles with correct masses", "[particle]") {
  std::vector<Particle> particles = create_initial_particles();
  std::vector<double> expected_masses = {
      1.0,           1.0 / 6023600, 1.0 / 408524, 1.0 / 332946.038,
      1.0 / 3098710, 1.0 / 1047.55, 1.0 / 3499,   1.0 / 22962,
      1.0 / 19352};
...
TEST_CASE("single body orbits around the Sun", "[solarSystem]") {
  ...
  Particle p1(Eigen::Vector3d(0.0, 0.0, 0.0), Eigen::Vector3d::Zero(),
              Eigen::Vector3d::Zero(), 1.0);
  Particle p2(Eigen::Vector3d(1.0, 0.0, 0.0), Eigen::Vector3d(0.0, 1.0, 0.0),
...
TEST_CASE("calculate kinetic energy", "[energy]") {
  Particle p(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(1, 0, 0),
...
TEST_CASE("calculate potential energy between two particles", "[energy]") {
  Particle p1(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0, 0, 0),
              Eigen::Vector3d(0, 0, 0), 1);
  Particle p2(Eigen::Vector3d(1, 0, 0), Eigen::Vector3d(0, 0, 0),
              Eigen::Vector3d(0, 0, 0), 0.1);
...
TEST_CASE("Energy calculation is correct (parallel vs serial)", "[energy]") {
  ...
  initial_total_energy_serial =
      initial_kinetic_energy_serial + initial_potential_energy_serial;
  ...
  final_total_energy_serial =
      final_kinetic_energy_serial + final_potential_energy_serial;
...
#pragma omp parallel for reduction(+:final_kinetic_energy_parallel, final_potential_energy_parallel)
  for (size_t i = 0; i < particles_parallel.size(); ++i) {
    ...
    for (size_t j = i + 1; j < particles_parallel.size(); ++j) {
      final_potential_energy_parallel +=
          calcPotentialEnergy(particles_parallel[i], particles_parallel[j]);
    }
  }
  final_total_energy_parallel =
      final_kinetic_energy_parallel + final_potential_energy_parallel;

  // Compare energies
  REQUIRE(std::abs(final_kinetic_energy_parallel -
                   final_kinetic_energy_serial) < 1e-10);
  REQUIRE(std::abs(final_potential_energy_parallel -
                   final_potential_energy_serial) < 1e-10);
  REQUIRE(std::abs(final_total_energy_parallel - final_total_energy_serial) <
          1e-10);
}
